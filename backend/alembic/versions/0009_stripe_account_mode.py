...
So the platform is recorded rather than inferred from the id. NULL means the
row predates these columns and is genuinely unknown, which is treated the same
as a mismatch: an unknown is never assumed to be the good outcome.

**Two columns, because neither alone is the identity.** A Stripe account keeps
one `acct_…` id across test and live, so `stripe_platform_id` does not tell the
two modes apart; and two different platforms in the same mode compare equal on
`stripe_account_livemode`. That second case is reachable from the order in
docs/LAUNCH.md — step 4 opens a *new* platform account under the new entity,
and testing that first means new **test** keys.
...
def upgrade() -> None:
    ...
    op.add_column(
        "cleaner_profiles",
        sa.Column("stripe_platform_id", sa.String(length=255), nullable=True),
    )


def downgrade() -> None:
    op.drop_column("cleaner_profiles", "stripe_platform_id")
